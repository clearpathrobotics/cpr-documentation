...
map details. The Operator can modify the map's name, default path properties and edit paths through this
view.
...
## Mission Creation  {#missionCreation}
...
<center>
  <figure>
    <img src="/img/outdoornav_images/ui_goalpoint_panel_2.png" />
...
as the mission name, and apply Tasks to when the Mission starts and stops.
...
4.  The check box next to the Task name controls mission behaviour in the event that the Task fails. If the checkbox is
    checked the Mission will proceed to the next step in its process, such as the next task or navigating to the next Goalpoint.
