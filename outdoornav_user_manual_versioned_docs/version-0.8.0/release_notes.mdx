...
-   Added localization status topic (see see [API Endpoints](api/api_endpoints/autonomy_api.mdx)).
-   Added re-localization service (see [API Endpoints](api/api_endpoints/autonomy_api.mdx)).
...
-   Goal terminology has been removed from the mission generation nomenclature (see 
    [Definitions](web_user_interface/ui_autonomous_mode.mdx#definitions)) 
