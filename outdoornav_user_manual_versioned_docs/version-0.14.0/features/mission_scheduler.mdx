...
- <strong>Start Time</strong>: The browser's local time that the schedule will start. This is stored as UTC time on the robot.
...
- <strong>Timeout</strong>: The maximum amount of time to spend on running a single loop of the schedule. If a loop exceeds this timeout value the schedule is assumed to have failed and alerts the user accordingly. This can be set to 0 to disable the timeout feature.
