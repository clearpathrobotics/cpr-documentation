...
Serial transport is more involved, requiring stream synchronization and integrity checking. See [the transport framing](./index.mdx#serial-transport) for more information on formatting.

The serial API is contains methods to populate the 6-byte header, including the CRC. It also contains functions to validate and de-payload data received from peers.
