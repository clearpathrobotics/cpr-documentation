...
**proton_ros2** is a ROS 2 adapter for Proton. It runs both a ROS 2 node and a `proton_cpp` node in the same process, and bridges Proton bundles to ROS 2 topics for publishers and subscribers, or services for servers and clients.
...
- Per-connection RX/TX rates (KB/s) from the embedded `proton_cpp` node
