...
Below are troubleshooting tips for Husky Observer. Refer also to [Husky Troubleshooting](../../../robots/outdoor_robots/husky/troubleshooting_husky) for
...
1. **When I start a Mission the robot does not move. What could be the problem?**
   ...
   1. Check that the wireless Motion-Stop is not engaged and the
      ...
      <span style={{ color: "#38761d" }}>GREEN</span> start button has been pressed to release the Motion-Stop.
...
1. **When trying to dock, my robot drives towards the dock but stops
   ...
   sections [Survey Base Station](user_manual_husky_observer#survey-base-station),
   [Set Datum](user_manual_husky_observer#set-datum), and
   [Set Dock Location](user_manual_husky_observer#set-dock-location).
