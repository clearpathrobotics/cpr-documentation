...
Built on top of the versatile [Husky](../../../robots/outdoor_robots/husky/user_manual_husky) base platform,
...
| Off                   | Magenta            | A system error/warning exists; see the [OutdoorNav UI](#monitoring-system-status) for details. |
...
Refer to [Husky Observer Integration](integration_husky_observer) for details on how to integrate your own payload or how to integrate a payload kit from Clearpath Robotics.
...
<center>
  <figure>
    <img
      src="/img/robot_images/husky_observer_images/autocharge_dock.jpg"
      width="400"
    />
    <figcaption>Autocharge Dock (TODO: new image)</figcaption>
  </figure>
</center>
...
<center>
  <figure>
    <img
      src="/img/robot_images/husky_observer_images/dock_space_requirements.jpg"
      width="600"
    />
    <figcaption>Dock Space Requirements (TODO: new image)</figcaption>
  </figure>
</center>
...
TODO: Update these installation details.

1. Open the lid on Husky Observer.
1. Find the Microhard.
1. Insert the SIM card.
1. See [Enabling LTE (Cellular) Networking on Robot](#lte-setup) for details on the software setup.
...
<center>
  <figure>
    <img
      ...
      width="600"
    />
    <figcaption>Robot main power switch (TODO: new image)</figcaption>
  </figure>
</center>
...
See [Maintenance](maintenance_husky_observer#charging) for details on charging the robot.
...
to the main computer. See [Husky Observer Integration](integration_husky_observer)
for further details.
...
[Husky Tutorials](../../../robots/outdoor_robots/husky/tutorials_husky#pairing-the-controller).
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ----------------------------------------- |
| Base Station     | Base Station with WiFi for near-range operations; optional LTE also available on robot for long-range operations | Pre-configured at factory | Pre-configured at factory; see also [Enabling LTE (Cellular) Networking on Robot](#lte-setup) for optional setup                                            | Pre-configured at factory                                                                                                                                                                                                                 | http://192.168.131.1          | See [Remote Access Setup](#remote-access) |
| Customer Network | Customer WiFi for near-range operations; optional LTE also available on robot for long-range operations          | N/A                       | See [Updating the WiFi SSID or Password on Robot](#robot-wifi-setup); see also [Enabling LTE (Cellular) Networking on Robot](#lte-setup) for optional setup | See [Windows 10 instructions](https://support.microsoft.com/en-us/windows/connect-to-a-wi-fi-network-in-windows-1f881677-b569-0cd5-010d-e3cd3579d263) for connecting to Customer WiFi network; configure as Static IP `192.168.131.75/24` | http://192.168.131.1          | See [Remote Access Setup](#remote-access) |
| Cellular Only    | Uses cellular in robot + separate cellular in Tablet                                                             | N/A                       | See [Enabling LTE (Cellular) Networking on Robot](#lte-setup) for setup                                                                                     | Pre-configured at factory (TODO: confirm) or use USB-tethering to a cell phone                                                                                                                                                            | N/A                           | See [Remote Access Setup](#remote-access) |
...
1. Open the following in your Chrome browser: http://192.168.131.50/network/wan<br/>
...
1. Update the "WAN2 Port Configuration" as desired. TODO: example figures (DHCP vs Static)
1. TODO: how to save/apply the changes?
...
1. Open the front panel of the Base Station and connect an Ethernet cable from
   the configured port on your laptop to one of the available LAN ports on the
   Microhard in the Base Station.
1. Open the following in your Chrome browser: http://192.168.131.50/wireless/radio1<br/>
...
1. Open the following in your Chrome browser: http://192.168.131.51/wireless/radio1<br/>
   This should bring you to the "Wireless &srarr; Radio1" menu.
   Note: Refer [Appendix A](#networking-configuration) for default credentials.
...
TODO: failover details; possibly port additional settings from https://wiki.clearpathrobotics.com/display/~stphillips/Microhard+Cellular+Setup.
...
1. Connect an ethernet cable from the configured port on your laptop to the
   debug/expansion port on the top of the robot.
1. Open the following in your Chrome browser: http://192.168.131.51/wwan/settings<br/>
   This should bring you to the "WWAN &srarr; Settings" menu.
   Note: Refer [Appendix A](#networking-configuration) for default credentials.
1. Apply the following settings: (TODO: image)
   ...
   - RSSI Threshold: `90`
1. After a couple minutes, check the WWAN status in your Chrome browser: http://192.168.131.51/wwan/status<br/>
   Ensure that "Activity Status" is `Connected` and that a "WWAN IP Address" has been assigned. (TODO: image)
1. Disconnect the Ethernet cable.
...
The Husky Observer ships with a Getac ruggedized tablet as shown in below.
...
once it has been surveyed. Alternatively, RTK corrections can be acquired through
the internet, such as through the [SwiftNav Skylark](https://www.swiftnav.com/skylark)
...
If a Base Station is not present, RTK corrections can be acquired through
the internet. While various services are available for providing RTK corrections,
...
autonomous missions are available to be downloaded from: `/home/administrator/missiondata`.

Key notes:

- In the above directory, there will be a subdirectory for each of the cameras.
  Each of the camera subdirectories will contain images and videos whose filenames
  are based on the time at which the image was captured or the time at which the video
  recording was started. For example, a video that was
  started on May 17, 2022 at 13:25:08 would correspond to a filename of:
  `20220517T132508.avi`
- Audio files use a similar file naming convention as images and videos.
...
be `customer1-sftp.ngrok.io`.

:::

To use FileZilla, begin by entering the following settings on a laptop or tablet
on the Base Station network, then click "Quickconnect" as illustrated in
the figure below.
...
<center>
  <figure>
    <img
      ...
      width="800"
    />
    <figcaption>FileZilla Quickconnect</figcaption>
  </figure>
</center>
...
TODO: Add notes about how to monitor the system status in the OutdoorNav UI once we implement this feature.
...
All images and videos that are collected during missions are stored on the robot Base Computer
until downloaded and deleted by the operator. When the download/delete function is performed
by a computer within the Base Station network, the robot Base Computer can be accessed
as described in the previous section by connecting to TCP port 22 on the robot Base Computer.
When the download/delete function is performed by a computer on the WAN network (connected
to the Base Station WAN port), the operator can access the robot Base Computer by accessing
TCP port 8022 on the Base Station, which gets mapped to TCP port 22 on the robot Base Computer.
...
- Resolution: Max: 1920x1080. ROS driver defaults: 640x480
- Frame rate: 60Hz
...
a laptop to the robot and assign the laptop a static IP address in the range `192.168.131.100-200`. Then open a
browser and navigate to [http://192.168.131.10](http://192.168.131.10).

ROS driver settings can be edited in `/etc/ros/noetic/ros.d/axis_ptz.launch` or by changing the environment variables
found in `/etc/ros/setup.bash` on the robot's primary computer.
...
To prevent damage to the robot, Clearpath recommends setting pan limits on the Axis camera. This will restrict the
camera's range of motion to less than 180 degrees in either direction, eliminating the risk of the camera making
continuous rotations.

To set pan limits, connect your laptop to one of the robot's exposed ethernet ports and assign your laptop a static
IP address in the range `192.168.131.100-200`. Open your web browser and navigate to
[http://192.168.131.10](http://192.168.131.10). Log into the Axis configuration website using the credentials found
...
Move the camera to a pan of -179 degrees. The easiest way to move the camera precisely is to launch the ROS driver and use the `cmd` topic to move the camera:
...
### FLIR Thermal Camera Configuration

TODO

### Shotgun Microphone Configuration
...
1. Base Husky software. Refer to the [Husky Tutorials](../../../robots/outdoor_robots/husky/tutorials_husky#installing-husky-software) for details.
...
See [Maintenance](maintenance_husky_observer#software_maintenance) for details on keeping Husky Observer software up to date.
