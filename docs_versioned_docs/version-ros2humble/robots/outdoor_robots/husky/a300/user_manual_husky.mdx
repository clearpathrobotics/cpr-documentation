...
robot is locked out. This terminals at the breaker and at the battery are protected with rubber covers.
...
- Husky A300 Standard Manual Charger (Henryuan Model TODO): 300 W, indoor use only
- Husky A300 High Power Manual Charger (Delta-Q Model TODO): 1200 W, indoor or outdoor use (IP68?)
...
any user intervention (TODO: LEDs; ONav UI link). When charging is complete, the transmitter
will stop charging automatically (TODO: LEDs). Note that when the robot is powered
...
once every three months, it battery levels checked, and if the reported charge level is below 20% it
