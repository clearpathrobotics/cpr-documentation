...
- Clearpath Robotics robots should never be driven onto curbs higher than 150 mm or across gaps or depressions in the ground that may result in
  damage to the vehicle drive systems, attachments, or accessories -- for example, at the top of stairs, at a cliff edge, or at large potholes.
...
TODO: Caution box: Loading a Clearpath Robotics product with more weight than specified by the attachment,
accessory, or base Clearpath Robotics robot (when no attachment is present) will prevent Clearpath Robotics robots from
correctly determining stopping distances and may also damage the vehicle, attachment, or
...
TODO: Warning box: Whenever completing inspections or maintenance on or around the areas of a Clearpath Robotics robot
...
Clearpath Robotics robot. Always be sure to lock-out the vehicle before proceeding with inspections and
maintenance.
...
- The robot is equipped with an electrical braking system that limits the maximum speed of the robot
  when engaging the Emergency Stop on an incline.
...
The chassis contains the robots batteries, electonics, and cooling systems. It is an IP54-rated enclosure.
...
| :---------------------------------------- | :----------- | :---------- |
| Idle                                      | Front LEDs solid white (low intensity), rear LEDs solid red (low intensity) | The robot is idle (normal operation). |
| Driving                                   | Front LEDs solid white (medium intensity), rear LEDs solid red (medium intensity) | The robot is driving (normal operation). |
| Low Battery                               | Pulsing orange | The robot's batteries are in a low charge state. Connect the robot to a charger. |
| Emergency Stop Engaged                    | Flashing red | Emergency Stop mode is engaged. To disengage, twist both red mushroom buttons on the robot to ensure that they are unlatched and press the Restart button. |
| Charging (wired or wireless)              | Pulsing green | The robot's batteries are being charger, either from the wired/manual charger or the wireless charger. |
...
| Battery Fault                             | Flashing yellow and red | An error has been detected with the battery. Please contact support. |
...
Refer to TODO for more details.
...
Note that the can be charged (manual and wireless charger options) regardless of the On/Off Breaker position.

Note also that Husky ships with the breaker in the "off" position to prevent battery drain during shipping.
...
Note that the charger also functions as "shore power", providing power to the robot indefinitely while connected.

#### Debug Ports

The robot is equipped with a Gigabit Ethernet port, a USB 3.0 (Type A) port, and an HDMI port. These can be used for debugging on the
robot without needing internal access to the robot. These ports are all connected directly to the robot's computer.
...
vehicle is locked out. This terminal is protected with a rubber cover (TODO: confirm). Do not remove this or any
other terminal covers. The main chassis will act as the negative side of the battery (TODO: confirm). It the vehicle
is correctly locked out, all live terminals are protected. The vehicle must be locked out to
