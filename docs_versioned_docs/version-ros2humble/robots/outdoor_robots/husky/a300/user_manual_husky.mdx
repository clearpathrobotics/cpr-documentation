...
For most Husky deployments, its built-in computer has a network (wired or wireless) to an Offboard Computer that is used to control Husky and gather data.

### Powering Up

To power on Husky:

- Open the Access Panel cover, set the On/Off Breaker to the "on" position, and close the Access Panel cover.
- Press and release the Power Button.
- Wait one minute for the robot to initialize fully. During this time, the Status Lights will be solid red.
...
To get Husky connected to your local Wi-Fi network, you must first access the built-in computer using a
wired connection to the robot.
...
Follow the instructions in the [Husky Tutorials](tutorials_husky) as needed.

To teleoperate Husky, first clear any stop conditions. Then, referring to the following images, press the
...
Husky A300 ships with its batteries installed and they are designed to be charged in place. The batteries
may be charged with the standard manual/wired charger with the optional wireless charger upgrade kit.

#### Manual/Wired Charging

The steps
to charge the batteries are outlined below. Note that the batteries can be charge regarless of whether
the robot is powered on or powered off, though the charge will progress more quickly when the robot
is powered off.
...
- Check Observer documentation
- Check OTTO documentation
- FCC/ICES section?
