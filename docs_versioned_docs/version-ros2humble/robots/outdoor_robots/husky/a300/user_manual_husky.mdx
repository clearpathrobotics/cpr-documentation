...
Husky's batteries are capable of delivering 1800 W (TODO: check), similar to wall mains.
...
| Storage Temperature            | -20 °C to TODO °C                                                        |
...
| User Power                     | 12 V @ 10 A, 24 V @ 5 A, VBAT @ 5 A  (TODO: confirm)                     |
| Environmental                  | IP54                                                                     |
...
Husky is built around a computer running Ubuntu (Intel-based computer or Jetson developer kit), paired with a 32-bit microcontroller unit (MCU).
...
The front and rear bumper bars not only offer protection for Husky, they can be used as lift points
for Husky (TODO: confirm) and are rated to four times the robot weight, including maximum rated payload
as specified in [System Specifications](#system-specifications). Refer to TODO for more details on how
to ensure system stability when lifting the robot by the bumper bars.
...
be disengaged and the Restart Button must be pressed. Refer to TODO for more details.

#### Restart Button

To clear the stop condition, all Emergency Stop buttons must
be disengaged and the black Restart Button (TODO: symbol to match robot) must be pressed. Refer to TODO for more details.
...
The rear of the robot has an access panel, protected by a lockout, that contains the Battery Charge Port, On/Off Breaker,
...
When equipped with a lock, the lockout prevents access to the On/Off Breaker, Battery Charge Port, and Debug Ports.
...
to the Battery Charge Port. Refer to TODO for additional information on charging the robot's batteries.
...
- Clearpath Robotics Husky A300 robot
   - Two or four LiFePO<sub>4</sub> batteries (integrated)
   - Computer (integrated)
- One battery charger
- One PS4 controller
...
TODO: Danger: The On/Off Breaker terminal connected to the battery (displayed below) is live even when the
...
TODO: Caution: Safety boots or shoes with steel toes should be worn at all times while working with a Husky A300.
...
1. Press and hold the Power Button for 1 second and then release it.
1. Wait one minute for the robot to initialize fully. During startup, the Status Lights will be solid red.
   ...
   The Status Display should also indicate that there are no errors (TODO: image) and that there are
   no communication errors (TODO: image).
...
TODO: Danger: Only original chargers from Clearpath Robotics should be used when charging the Husky A300 to
...
The steps to charge the batteries are outlined below. Note that the batteries can be charge regardless
of whether the robot is powered on or powered off, though the charge will progress more quickly when the
...
1.  Plug the charger power cord into the charger (if applicable) and then into a wall receptacle.
    The LED on the charger will illuminate TODO.
1.  When battery is fully charged, the LED light will TODO.
...
resist and attempt to hold its position, possibly resulting in damage to the robot. Pushing an unpowered
robot at normal speeds will not damage it.

To shut down Husky and move it manually:

1. Follow the steps in [Shutting Down](#shutting-down), including the Lock-Out/Tag-Out procedure.
...
TODO: Warning: Pushing a Husky using a powered device to push it at higher speeds, such as
...
1. Set the computer's IP address and netmask as shown below. For more details,
   refer to TODO tutorial.
...
1. Confirm that you can ping Husky from the computer's terminal window in Linux or the
   ...
   TODO: image of ping command.
1. TODO: SSH to the computer; covered in tutorial?
...
is used for most deployments.

##### Configuring Husky as a Wi-Fi Access Point  {#wifi-access-point}

TODO: Do we want to make this option available? How would the user switch to this mode?

In access point mode, a computer can connect to Husky using the following
settings:

- SSID: `Husky-A300-<serial_number>`, where `<serial_number>` is the serial number shown
  on the robot's label. TODO: where is the label
- Password: `clearpath`
- Computer Wi-Fi interface set up with DHCP enabled (to have IP address assigned automatically)
- `ping 10.42.0.1` to confirm access to Husky from the computer

To switch from client mode to access point mode:

- TODO: how to switch
- See ros/networking/computer_setup#custom-networking as backup

##### Connecting Husky to a Network over Wi-Fi

To get Husky connected to your local Wi-Fi network:

1. Following the steps for [Connecting to Husky over an Ethernet Connection](#ethernet-connection).
1. TODO: any steps for selecting client mode rather than access point mode?
1. TODO: edit `/etc/netplan/60-wifi.yaml`?? SSID + password


See the [Husky Tutorials](tutorials_husky) for details.
...
To teleoperate Husky, first clear any stop conditions. Then, referring to the following images, press an
enable button (either slow or fast mode) and use the left thumbstick to drive Husky.
