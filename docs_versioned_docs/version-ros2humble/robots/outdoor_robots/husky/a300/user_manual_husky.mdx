...
- Prior to performing maintenance, Husky A300 should be powered off and the On/Off Breaker set to the OFF position.
- If any fluids are spilled while maintaining Clearpath Robotics robots, they should be cleaned up in accordance with facility guidelines priority to operating the robot.
...
payload documented on the Nameplate.
...
When Clearpath Robotics Husky A300, ensure that you follow all guidance on allowable hill angles, allowable
...
Prior to operating Clearpath Robotics Husky A300, ensure that either:

1. there are no hazards in the environment due to Husky A300 rolling down a slope/ramp; OR
1. the combination of payload mass and slope angle are within the allowable
   stopping distance as defined in [Important System Limits](#important-system-limits).
...
Refer to [Important System Limits](#important-system-limits) for more details on stopping distances when
engaging the emergency stop system as a function of the incline and payload.
...
<center>
  ...
  <figure>
    <img
      ...
      width="500"
    />
    <figcaption>(TODO: A300 image) Wireless motion-stop override</figcaption>
  </figure>
</center>
...
continuous output for the standard and upgraded battery configurations is 60 A and 120 A, respectively.
Higher outputs are possible for short periods of time. 

Husky A300's motors are rated to draw (TODO) A continuous, but they will spike to several times higher
...
when the a maximum payload is added with its centre of mass at 50 cm above the Husky A300 origin point.
...
the speed of the robot. Depending on the payload mass and on the incline on which the robot exists,
the robot may stop completely or reach a terminal speed when the emergency stop is engaged.
...
| Payload Mass (kg)  | Incline (°)  | Stopping Distance (m), assuming 2 m/s when emergency stop engaged | Terminal Speed (m/s)  | 
| :----------------- | :----------- | :---------------------------------------------------------------- | :-------------------- |
| 0                  | 15           | TODO                                                              | 0                     |
| TODO: max payload at which we stop at max incline                | 15           | TODO                                                              | 0                     |
| 100                | 0            | TODO                                                              | 0                     |
| 100                | TODO: max incline at which we stop with max payload         | TODO                                                              | 0                     |
| 100                | 15           | TODO                                                              | TODO                  |

##### Other Limits

   - TODO: Hardware failures?
   - TODO: Bandwidth? Loss of networking?
...
| Operating Time, Average⁵       | 4 hours (standard battery) or 8 hours (upgraded battery)                 |
| Operating Time, Standby        | 12 hours (standard battery) or 24 hours (upgraded battery)               |
| Battery                        | 40 Ah @ 24 V (standard) or 80 Ah @ 24 V (upgraded), LiFePO<sub>4</sub>   |
| Standard Battery Charger       | 10 A; short-circuit, over-current, over-voltage and reverse voltage protection |
| Charge Time⁶                   | 3-4 hours (standard battery) or 7-8 hours (upgraded battery)             |
...
- Status lights
- Buttons
- Protected Access Panel (On/Off breaker, battery charge port, HDMI/USB/Ethernet debug ports, wireless emergency stop bypass)
...
Note that the Power button has no effect when the On/Off breaker is in the OFF position, unless
...
TODO: rename to Battery Disconnect Breaker (or something similar)
...
Contact [Sales](#sales) for details on how to upgrade to a safety-rated autonomy system for
Husky A300.
...
1. TODO: will the user need to press the Safety Restart button (similar to OTTO 100) each time after power up?
...
The batteries can be charged when the robot is powered on or powered off, though the charge will progress more quickly when the
...
TODO: User manual to include test results of thermal testing per IEC 61010-1
TODO: User manual to include test results of EMC emissions testing per IEC 61326-1
