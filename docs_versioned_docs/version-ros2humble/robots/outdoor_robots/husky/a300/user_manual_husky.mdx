...
- orange: Pinch/Crush
- orange: Tripping Over Robot: outline safe parking procedures
- orange: Tipping: include conditions for use related to allowable hill angles, allowable payload mass, and allowable payload location; interface stability cone
   - Negative obstacles
   - Tips sideways on slope
   - Tips forward or backward on slope
- orange: unpowered robot rolling down a ramp
- orange: E-stopped robot rolling down a ramp, steep ramp or heavy payload
- orange: Health issues due to chemicals in thermal paste or thread locking adhesives: wear nitrile gloves while maintaining the robot
...
As described further in [Integration](integration_husky#wireless-stop), it is possible to add a wireless
emergency stop device to the system. In some debugging scenarios, it may be desirable to bypass / ignore
the input from the wireless emergency stop device. This bypass is controlled through a key switch as
illustrated below.
