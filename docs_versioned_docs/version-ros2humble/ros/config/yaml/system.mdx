...
```yaml
  hosts:
    - hostname: cpr-a200-0000 # The main robot PC
      ip: 192.168.131.1
    - hostname: my-remote-pc
      ip: 192.168.131.5
```
...
|:---:| :--------------: | ----------- |
| **username**   | string    | Must match the username that is used to run all ROS nodes. |
...
that this robot is connecting to but also every discovery server that the remote PC will connect to in the system.
