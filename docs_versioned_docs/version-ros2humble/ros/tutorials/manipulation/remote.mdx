...
## Launching RViz
Once MoveIt! is running, to open RViz with the MoveIt MotionPlanning plugin, with the appropriate namespace, and interactive markers:
```
ros2 launch clearpath_viz view_moveit.launch.py namespace:=''
```
> Make sure to pass the same namespace as the robot's
