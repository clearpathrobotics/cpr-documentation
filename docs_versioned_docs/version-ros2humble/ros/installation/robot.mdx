...
When prompted to set the username to `administrator`.
...
Once the firmware has been flashed, place the Jackal MCU back into normal mode by switching the PWR_MODE switch fromALT to NORM.
...
Once the Clearpath package server is set up, follow the [Clearpath Computer Setup](../networking/computer_setup)
instructions to configure the network interfaces.
