...
:::note
If the wifi interface indicates down (disabled), enable it using `sudo ip link set dev <interface> up`.
...
For details on setting up custom network interface settings see [Clearpath Computer Setup](../networking/computer_setup).
...
**2. Create a workspace on the robot's computer for the clearpath packages**
...
in the [ROS2 Environment](../config/yaml#ros-2-environment) section.
:::
