...
The `backup.sh` script assumes that your robot is in a roughly-standard configuration; it uses a single user account and no files within `/opt/ros/$ROS_DISTRO`have been modified.
...
The environment variables used to define your robot in ROS 1 and ROS 2 Foxy can be converted to the `robot.yaml` format to maintain the same robot description when you upgrade to ROS 2 Humble.
Below are some common examples. Visit the [robot.yaml documentation](../config/yaml.mdx) for more details.

### Attachments

Each platform has platform-specific attachments (formerly decorations) such as bumpers and top plates. These now reside under the [platform](../config/yaml.mdx#platform) section of the `robot.yaml` file.
...
platform:
  attachments:
    front_bumper:
      ...
      model: wibotic          # HUSKY_WIBOTIC_BUMPER=1
      xyz: [0.0, 0.0, 0.0]
      rpy: [0.0, 0.0, 0.0]
    ...
    top_plate:
      ...
      rpy: [0.0, 0.0, 0.0]
    structure:
      enabled: true           # HUSKY_SENSOR_ARCH=1
      model: sensor_arch_510  # HUSKY_SENSOR_ARCH_HEIGHT='510'
...
platform:
  attachments:
    front_fender:
      enabled: true
      model: sensor         # JACKAL_FRONT_ACCESSORY_FENDER=1
      xyz: [0.0, 0.0, 0.0]
      rpy: [0.0, 0.0, 0.0]
    rear_fender:
      enabled: true
      model: default        # JACKAL_REAR_ACCESSORY_FENDER=0
      xyz: [0.0, 0.0, 0.0]
      rpy: [0.0, 0.0, 0.0]
```
  </TabItem>
...
Sensors can be added under the [sensors](../config/yaml.mdx#sensors) key in the `robot.yaml` file. There is no longer a concept of primary, secondary, etc. sensors. Rather, sensors are enumerated in the order that they are added to the config.
