...
Check the [list of supported platforms](../../ros#supported-platforms) to make sure that your robot is currently
...
variables (e.g. `JACKAL_URDF_EXTRAS`) and have stored them outside any of the folders specified above, you must
...
now reside under the [platform](../config/yaml.mdx#platform) section of the `robot.yaml` file.
...
Sensors can be added under the [sensors](../config/yaml.mdx#sensors) key in the `robot.yaml` file. There is no longer
