...
To evaluate the network, it is best to start by stopping all ROS systems on the network and then using [iperf3](network_troubleshooting_tools.mdx#iperf3) to verify the network bandwidth. This should be done with the robot in several locations, particularly any location where the instability issues occur. In general, as a rule of thumb, if the bandwidth is less than 40 Mbps then it is a low bandwidth network and it must be very carefully managed to support a ROS 2 system.
