...
:::warning not available without pacs:
 At this time, you will **need to use the PACS™ top plate**, only available using the `a200.top_plate` with `model: pacs`, to add these riser mounts. See the [**platform**](./platform/attachments) section for more information.
:::
