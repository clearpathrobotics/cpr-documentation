...
sidebar_label: Robot Installation
sidebar_position: 2
...
Clearpath Robotics' [ROS 2 Jazzy ISO image](https://packages.clearpathrobotics.com/stable/images/2.0.0/)
**will wipe the robot's drive** when used.

:::

:::note

If your robot has multiple internal drives, or if you need more control over partitioning the drive (e.g.
you want to keep `/` and `/home` on different partitions of the same drive) you should use the normal
[Ubuntu Server 24.04 ISO](https://ubuntu.com/download/server/thank-you?version=24.04.2&architecture=amd64&lts=true)
and then use [Clearpath's robot installation script](https://github.com/clearpathrobotics/clearpath_computer_installer)
to configure the computer after Ubuntu is installed.

:::

<Tabs>
  <TabItem value="Clearpath ISO" default>
Clearpath provides a lightly customized installation image of Ubuntu 24.04 that has the needed script to setup the robot.
...
1.  Download the [ROS 2 Jazzy ISO image](https://packages.clearpathrobotics.com/stable/images/2.0.0/).

2.  Copy the image to a USB drive using `unetbootin`, `rufus`, `balena etcher`, or a similar program. For example:

    ```
    sudo unetbootin isofile="clearpath-noble-jazzy-autoinstall.iso"
    ```
...
has multiple drives, you can use the official Ubuntu Server ISO from Canonical. This ISO has more options
available than the Clearpath ISO.

To install the software you will need a USB drive of at least 4 GB to create the installation media, an ethernet
cable, a monitor, and a keyboard.

1.  Download the [Ubuntu Server ISO image](https://ubuntu.com/download/server/thank-you?version=24.04.2&architecture=amd64&lts=true).

2.  Copy the image to a USB drive using `unetbootin`, `rufus`, `balena etcher`, or a similar program. For example:

    ```
    sudo unetbootin isofile="ubuntu-24.04.2-live-server-amd64.iso"
    ```
...
3.  Connect your robot computer to internet access (via wired Ethernet), a keyboard, and a monitor.
    ...
    The next step will involve reformatting some or all of the partitions on your robots hard drive. Make sure all
    important data is backed-up before proceeding.

    :::

4.  Boot your robot computer from the USB drive and follow the on-screen instructions to install Ubuntu Server 24.04.
...
5.  Allow the setup process to complete. Depending on your internet connection's speed this may take some time.

6.  When prompted, remove the USB drive from the computer and press the `ENTER` key to reboot.

7.  On first boot, log into the robot using the username and password you configured during step `4`. To finish
    configuring the computer for use with a Clearpath robot, run the following commands:

    ```bash
    sudo apt-get update
    sudo apt-get install wget
    wget https://raw.githubusercontent.com/clearpathrobotics/clearpath_computer_installer/refs/heads/main/clearpath_computer_installer.sh
...
You should now be able to SSH into the robot by calling `ssh robot@<robot_ip>` from an offboard computer.
...
All platforms with the exception of the A200 will require a firmware update to run ROS 2 Jazzy.
...
is found in the [Robot Configuration](../config/yaml/overview) section. This YAML is used to generate the launch,
...
<details>
  <summary>Click to expand the manual source install instructions. Only use this option if you
  ...
  for installing ROS 2 Jazzy from debian packages.
  ...
  **3. Update your computer’s package cache**
  ...
  ```
  sudo apt install ros-jazzy-clearpath-robot
  ```
  ...
  **4. Link the workspace**
  ...
  ```
  source /home/robot/clearpath_ws/install/setup.bash
  ```
  ...
  Create the setup folder where the [robot.yaml](../config/yaml/overview) file will be stored, and where files will be generated:
