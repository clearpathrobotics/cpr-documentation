...
Check the [list of supported platforms](docs/ros/supported-components) to make sure that your robot is currently
supported on ROS 2 Jazzy.

:::

ROS 2 Jazzy is the first ROS distribution that runs on Ubuntu 24.04. As a result, any robots running ROS 1,
ROS 2 Foxy, or ROS 2 Humble will have upgrade their operating system. It is highly recommended to back up any
important files on the robot before proceeding, especially for highly customized robots.
...
the robot computer's hard drive, reserving the drive and installing a new one to use with Jazzy.
...
Please save the files listed above and use them as a reference during Jazzy configuration if upgrading from ROS 1.
...
Below are some common examples. Visit the [robot.yaml documentation](../config/yaml/overview) for more details.
...
now reside under the [platform](../config/yaml/platform/attachments/overview) section of the `robot.yaml` file.
...
Sensors can be added under the [sensors](../config/yaml/sensors/overview) key in the `robot.yaml` file. There is no longer
