The Clearpath robot packages currently support eProsima Fast DDS as the ROS Middleware (RMW) Implementation.
This middleware has two supported options for ROS 2 node discovery, Simple Discovery and Discovery Server.
Each of these discovery mechanisms has their own use case:
...
**Discovery Server**
- Requires [manual configuration](/docs_versioned_docs/version-ros2jazzy/ros/networking/ros2_networking/ros2_discovery_config#discovery-server-configurations) in `robot.yaml`
