...
where `license_key` is the the license key that you have been assigned. Only one license key can be used per UGV.
...
To start up the outdoronav software run:
...
To stop the OutdoorNav sotfware run:
