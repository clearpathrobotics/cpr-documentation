...
- Added ability to dock/undock via missions and tasks in the UI.
- Wireless indicator added to UI.
- Added collision avoidance toggle to the UI.
- Run OutdoorNav from only the autonomy docker image
- Getting started guide updated in the documentation.
- Added official OutdoorNav support for Jackal.

### API Changes

- [Github repository](https://github.com/cpr-application/clearpath_outdoornav) created for API users
- [API documentation](./api/api_overview.mdx) is now linked to new Github repository.

### Bug Fixes

- Reorder BMS topics in UI.

### Known Issues 

- Cellular indicator in the UI not functioning.
- Oak-D Pro PoE camera URDF not showing up in visualization.

## 2.2.0

### New Features

- Added custom tasks to the API and UI. Users can now create their own custom tasks to be added to missions through the UI or API.
- Added task adaptors to enable tasks to be added to missions through the UI.
- Added GNSS/RTK watchdog
- Added autonomy control of status lights

### API Changes

- Added current_goal_id topic

### Bug Fixes

- Fixed docking timeout parameter
- Duplicating mission should not duplicate tasks
...
- Added logger node to record execution logs on the robot. Logs include all mission data, images, video/audio recordings as well as telemetry and GPS data.
  Details of the logs can be viewed through the UI or you can access the file browser.
- Removed onav-backup node

### API Changes

- Updated docker images to confirm with Clearpath API

### Bug Fixes

- Fixed autonomy/status pause field not working

### Known Issues 

- Docking timeout not working
- Duplicating mission should not duplicate tasks
...
- First release with ROS 2 Jazzy support

### API Changes

- Many changes as part of the migration to ROS 2; not backwards compatible with earlier releases
...
- Version bump to 1.0.0 to reflect the last official ROS1 release.
  This version includes all features and bug fixes from the previous version.
...
- System Watchdogs can now be added to the UGV to 
...
- Points of Interest can now be added either
  through the UI or through the API to specify locations/objects of interest that
  are persistent.
- Execution Logs are now recorded and stored on the UGV.
...
- Updated /safety/safety_stop message type
...
- Operators can now Schedule Missions to run at specific times
...
- Create custom tasks that can be run during missions
- If installed with PDU, UGV can be set to Low Power mode to better conserve power
- New navigation topics added to ROS Autonomy API:
  - /navigation/progress
...
- Added localization status topic.
- Added re-localization service.
...
- Goal terminology has been removed from the mission generation nomenclature.
