...
## Switching Between Versions of OutdoorNav Software

If multiple versions of OutdoorNav software are installed, the `onav upgrade`
command can be used to switch between versions. For example, to switch from
0.12.0 to 0.11.0 (assuming that both are already installed), run:

```
onav upgrade 0.11.0
```

To switch back to 0.12.0, run:
