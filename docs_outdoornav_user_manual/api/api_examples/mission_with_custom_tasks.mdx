...
Before being able to run the examples similar to the one explained below, it is reuired that you have written your own custom tasks. See the
[Custom Tasks](/docs_outdoornav_user_manual/features/custom_tasks) section for information on how to develop tasks for your application.
...
import rospy
import actionlib
from cpr_outdoornav_api_examples_lib.coordinate_lat_lon import CoordinateLatLon
...
Import the `RosNode`, `CoordinateLatLon`, `Waypoint` and `Mission` classes which will be used to create the mission to be executed. 
