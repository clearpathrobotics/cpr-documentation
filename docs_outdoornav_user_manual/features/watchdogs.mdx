...
As of version 0.14.0, the OutdoorNav software contains the ability for users to set
up Points of Interest (POI) on their map. Each UGV will come pre-loaded with a set of
watchdogs that are required for the overall safety of the system. Users are then able
to add/remove/modify watchdogs as they require for their specific application and
...
to the user for more precise debugging of when issues may have occured.
...
| **Heartbeat Watchdog**           | A watchdog used to monitor the heartbeat topic of a device. This differs from the Communication, Node Status and Topic Data watchdogs because there may be devices that are deemed to be alive and communicating by the Node Status and Communication watchdogs and but may not be publishing data on their published topic. FOr example some GNSS units will not publish to their if topic unless they are in a clear open environment, however the heartbeat topic will still be publishing since the node is alive. |
...
| **Node Status Watchdog**         | A watchdog used to monitor the status of a node. It will trigger is the node crashes or does not start up. |
...
| **Num Points Watchdog**          | A watchdog used to monitor the status of a LiDAR device. It is designed to trigger if there are insufficient number of points in a PointCloud2 topic. The primary example for such a watchdog is if a customer has forgotten to remove a protective covering from the LiDAR. |
| **Odometry Covariance Watchdog** | A watchdog used to monitor reliability of the odometry data that is computed by our system. It will trigger if the any of the odometry covariance thresholds have been exceeded. |
...
This will open up a pop up. Here, the user can enter the required information related 
creating a new watchdog. The default parameters that need to be entered are:

- **Watchdog Name**: A semantic name for the watchdog.
- **Autonomy Action**: The type of action that the autonomy should perform when the watchdog
is triggered. Options are: "Stop", "Pause", "Warn"
...
- **Watchdog Type**: Selecting the required watchdog type will then prompt the user
...
example of modfying the properties of the TopicDataWatchdog. To confirm and save the changes to
