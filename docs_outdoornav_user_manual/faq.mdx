...
1.  **How do I start, pause or stop a mission?**
    ...
    A mission can be started in a few different ways, i) through the web user interface (See [Mission Execution](./web_user_interface/ui_autonomous_mode#mission-execution)) or,
    ii) through the ROS API (See [API Examples](./api/api_examples)).
...
3.  **What version of ROS is compatible with OutdoorNav?**
    ...
    Currently OutdoorNav is developed in ROS1 (and is built in DOcker containers on top of ROS1 Noetic).
    So, the recommended ROS version is ROS1 Noetic. However, although not recommended, it is still
    possible to use ROS1 Melodic. Some issues may arise related to the mismatched message types.
    Also, if trying to run any python scripts as Melodic targets Python2, whereas Noetic targets Python3.

    Finally, official ROS2 compatibility is currently unsupported. In theory, if you only have ROS2 installed
    on your system, you can try to install ROS1 Noetic as well and set up a ros1_to_ros2 bridge. Since the 
    OutdoorNav software is packaged in Dockers, it will communicate directly ROS1 and then the data can be
    redirected by the ros1_to_ros2 bridge to ROS2.

4.  **How can I record ROS data?**

    There are two ways to record ROS data. The first is [through the UI](./features/rosbag_recorder) and the
    second is the more traditional method of accessing the command line of the UGVs PC and running 
...
5.  **Where do all the video/audio/images get saved from the mission tasks?**
    ...
    UGVs PC at the following location: `~/clearpath_files/...`
...
1.  **Am I able to send the UGV on a repeated loop?**
    ...
    location and the the scond to return to the starting point). With these two missions you will then either be
    able to send each mission one after the other or you can use the [Mission Scheduler](./features/mission_scheduler).
...
2. **Why is surveying failing?**
    ...
    -   From the robot PC, check that you can ping the base station.

``` bash
ping 192.168.131.30
```

&emsp;&emsp;If all of these tests PASS, contact [customer support](./support).
...
3.  **Is it possible to increase the maximum velocity of the UGV?**
    ...
    [tuning guide](./customized_tuning/tuning_instructions/differential_drive_dynamics) and contact
    [customer support](./support) if required.

4.  **My robot it detecting too many obstacles and replanning too frequently?**

    Please consult the [Limitations](./features/collision_avoidance#limitations) section of the collisison
...
5.  **I am getting a 'Robot too far off path warning'. What should I do?**
    ...
    These types of warnings appears on the UI under two conditions:
    i)  If the robot indicator (blue arrow on the UI) is not within 3.0 meters of the first Waypoint (the Green one on the UI).
    ii) If the robot is outside of the allowable navigable space (defined by the path contraint, defualt: 3.0 meters around the reference path). 
    Enable the visualization of the [path contraint](./web_user_interface/ui_autonomous_mode#constrained_path) Teleop the robot back into the the 

6.  **How can I remove certain sensors from the collision avoidance pipeline?**

    If your robot is equipped with collisions avoidance sensors (eg. Velodyne, Hokuyo, Sick LMS), it is
    possible to enable/disable the throughput of each sensor data into the collision avoidance pipeline.
    Consult the [Collision Avoidance](./features/collision_avoidance) section for these instructions.

7.  **Can I use OudoorNav without using the UI?**

    We empower customers to send mission via either the UI or our [ROS1 API](./api/api_overview). Through this
    API, you are enabled to write Python scripts where you would generate your mission, survey the base station,
    set the datum, assign [custom tasks](./features/custom_tasks) to your mission. See some of the 
    [example codes](./api/api_examples) for an idea of how to generate these files.
...
2.  **How do I add a Task to a Waypoint on the UI?**
    ...
    See [Add Task to Waypoint](./web_user_interface/ui_autonomous_mode#add-task) for information on how
...
4.  **Are we able to make any changes to the UI?**
    ...
    This feature will be available in a future release. Please direct any feature requests to [customer support](./support).

5. **What to do if my custom task is not working?**

    Ensure that you have followed the [Custom Tasks](./features/custom_tasks) instructions. When adding the
    custom task to a Waypoint, it is important to double check that all the [fields](./web_user_interface/ui_autonomous_mode#add-task) 
...
6.  **Why are either of the GNSS Status indicator (POS and/or DIR) on the UI not Green?**
    ...
    i)  If the **POS** indicator is YELLOW, and you are using an RTK base station:
        -   Check communication between the robot and the base station by pinging 192.168.131.30
    ...
    ii) If the **POS** indicator is RED:
        -   Check communication between the robot and the position GNSS unit by pinging 192.168.131.31
        -   Access the [Swift Console](https://support.swiftnav.com/support/solutions/articles/44001391679-swift-console) using
