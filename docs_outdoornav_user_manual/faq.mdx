...
2. **Why is surveying failing?**
    ...
    -   Check that the base station is powered on, nd that all the devices inside are powered on.
...
5. **What to do if my task is not working?**

    Ensure that you have followed the [Custom Tasks](./features/custom_tasks) instructions. When adding the
    custom task to a Waypoint, it is important to double check that all the [fields](./web_user_interface/ui_autonomous_mode#add-task) for the new task are correct.
