...
| **`HYSTERESIS`** | State when you the platform requires compensation for tire flexion. | Will enter HYSTERESIS mode when the platform detects that it has will begin oscillating due to tire deformation. |
...
| `crosstrackerror_slowdown` | Threshold on the amount of crosstrack error, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
| `headingerror_slowdown` | Threshold on the amount of heading error, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
| `curvature_slowdown` | Threshold on the path curvature, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
...
| `corner_detection_threshold` | Threshold on the path curvature, between the platforms current location and the end of the corner_lookahead distance, above which to switch the MPC in state: CORNER, and begin reducing the horizon accordingly. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location)| **rad** |
