...
| **`RESCUE`**     | State to rescue the tracker when the platform is stuck.             | Will enter RESCUE mode when platform is appraching a condition that make it stuck.|
| **`HYSTERESIS`** | State when you the platform requires compensation for tire flexion. | Will enter HYSTERESIS mode when the platform detects that it has will begin oscillating sue to tire deformation. |
...
The following list defines the deafult parameters that the MPC controller uses. 
For the most part, they apply to all states of the MPC controller.

| Parameter | Description | Namespace | SI Units |
|-----------|-------------|-----------|:--------:|
| `vehicle_length` | The length (longitudinally) of the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
| `mpc_horizon` | The prediction horizon of the MPC. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **s** |
| `max_lookahead` | Maximum distance along the path we seek to follow. Affects how far we do collision checks as well as the maximum velocity of the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
| `min_lookahead` | Minimum distance along the path we seek to follow. Affects how far we do collision checks as well as the maximum velocity of the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
| `lookahead_smoother` | Factor between 0 and 1 that determines the smoothness of the change in lookahead distance: 0 means only maximum velocity is used to determine the horizon (can be jumpy but speeds up the platform faster); 1 means only averaged planned velocity is used to determine the horizon (smoother but speeds up the platform slowly). | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **--** |
...
| `min_fwd_velocity` | The minimum allowable linear velocity, in any mode, that the MPC controller can compute. This can be used to overcome the different deadbands that different platforms may have. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m/s** |
...
| `stiction_compensator_fwd` | The minimum linear velocity for movement of the platform. This should typically be the minimum linear velocity that the platform requires for it to move. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m/s** |
| `stiction_compensator_yaw` | The minimum angular velocity for movement of the platform. This should typically be the minimum angular velocity that the platform requires for it to move. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m/s** |
...
| `crosstrackerror_slowdown` | Theshold on the amount of crosstrack error, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
| `headingerror_slowdown` | Theshold on the amount of heading error, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
| `curvature_slowdown` | Theshold on the path curvature, above which to a slowdown the platform. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) |  |
...
| `shrinking_horizon_min` | The minimum horizon that will be used in the computation. This value will prevent the horizon from dropping below this value suring all the horizon adjustments. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **s** |
| `crosstrack_tolerance` | The amount of lateral error that the controller will handle before stopping the platform and replanning a new path. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
| `reference_trajectory_factor` | A factor that will skew the path parameter. This value should be between 0 and 1. Values closer to 0.0 will skew the path param closer to the platform, decreasing speed but increasing the accuracy of the path tracking, and vice versa as you approach 1.0. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
...
| `goal_slowdown_multiplier` | The multiplier by which to increase the MPC horizon during the MPCs GOAL state in order to slow the platform down from its top speed. This value should be greater than 1.0. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | |
| `max_speed_at_goal` | The maximum allowable speed at the goal point for the controller to stop and consider the goal complete. On OEM platforms, this value shold be increased. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m/s** |
...
| `corner_lookahead` | The distance on the reference path from the platforms current location, along which to determine if a corner is present. If a corner is present the MPC state will switch to CORNER state and begin reducing the horizon accordingly. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location)| **m** |
| `corner_detection_threshold` | Theshold on the path curvature, between the platforms current location and the end of the corner_lookahead distance, above which to switch the MPC in state: CORNER, and begin reducing the horizon accordingly. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location)| **rad** |
| `corner_slowdown_multiplier` | The multiplier by which to increase the MPC horizon during the MPCs CORNER state in order to slow the platform down from its top speed. This value should be greater than 1.0. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location)| **--** |
...
When tuning the controller on a third-party OEM platform, there are several other considerations
that we will not have taken into account during the tuning of our platforms. Below, we define
parameters that may be modified to tune the controller for your third-party OEM platform.
...
| `pivot_point_offset_x` | The longitudinal offset that can be applied to the MPC model in order to account for a particular platforms center of rotation | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | - |
| `pivot_point_offset_y` | The lateral offset that can be applied to the MPC model in order to account for a particular platforms center of rotation | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | - |
| `min_fwd_turn` |  | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **--**|
...
| Parameter | Description | Namespace | SI Units |
|-----------|-------------|-----------|:--------:|
| `enable_delay_compensation` | A boolean flag to enable/disable the delay compensation feature. The delay compensation feature is used to compensate for low-level dynamics delays that may be present in the platform to be controlled. The user can apply an input controller delay and the feature will compute command velocities that compensate for such a delay. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **bool** |
...
| Parameter | Description | Namespace | SI Units |
|-----------|-------------|-----------|:--------:|
| `enable_stop_distance` | A flag to use enable/disable the stop distance feature. The stop distance feature forces the platform to stop a specified distance in front of obstacles. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **bool** |
| `obstacle_stop_distance` | The distance at which the platform will stop in front of a detected obstacle. | [/navigation/*&ltcontroller&gt*/path_tracker](#file_location) | **m** |
