...
1. A single Waypoint that is 10-20 meters directly in front of the UGV. This will verify 
the requried linear motion of the UGV.
2. A single waypoint that is 10-20 meters directly behind the UGV. The first part of the 
navigation to this point (ie. the rotation to face the Waypoint) will verify the required 
angular motion of the UGV.
...
position. The four UGV orientations for each test are as follows:
...
angular:
  ...
  z: 0.0" -r 10
```
4. Analyse the motion and then stop the stop the linear motion. If the UGV drifts left/right
...
To test this we simply want to create a mission that sends the UGV to a single Waypoint 
directly in front of the UGVs current position. The Waypoint should be at least 20 meters
...
|-----------|-------|---------|
| [`max_speed_at_goal`](../tuning_parameters/navigation_parameters#goal_state_params)  | *depends on the deceleration of your UGV* | 0.8 |
...
If your UGV has RTK corrections, then it is only required to modify [`goal_tolerance_xy_rtk`](../tuning_parameters/navigation_parameters#goal_state_params)
