...
10. **Stack Light:** If a Stack Light is configured for the UGV it's current status 
    will show here.  
...
12. **Lights:** Any light controls that are setup on the UGV can be controlled through 
    a simple drop down button as well as relevant hotkeys, depending on the configuration.

13. **Record Audio:** If the UGV is equipped with a microphone the user 
    can start/stop recording manually by clicking this icon.

14. **Mission Execution Buttons:** These buttons allow users to Start, Pause and Stop an 
    autonomous mission.
