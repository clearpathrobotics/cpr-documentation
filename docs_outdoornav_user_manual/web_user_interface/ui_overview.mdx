...
Users are now able to add Points of Interest (POI) onto their map, marking a location or and object that is persistent
across both "Map" and "Waypoint" planning modes. The intended use for the points of interest feature is to senable the user to
set up locations that they know will almost never change and be able to for example navigate, inspect these locations.
...
of the POI. If you want to simply modify the heading of a POI, you can use the CTRL hotkey to enable heding modification
and rotate the POI to your dsired heading. See video below for an example of the editing the heading of a POI using the 
(CTRL hotkey).
...
If you, for example, have a patrol application in which you want your UGV to always navigate to a set of predefined locations.
You can do such a thing by creating POIs for each of the stops that the UGV should make. The by entereing the network mission
edit mode (hotkey: e+3), the user can simply click on the POIs in order of traversal to create your network mission. The network 
goals that are created on each click of a POI will place a Goal at the exact location of the POI and will also use the heading
that has been defined by the POI.
