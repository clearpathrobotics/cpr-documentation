...
12. **Edit Mission Toggle:** This toggle allows the user to start
    creating Viapoints for the current Mission. Once this button is enabled,
    the Waypoint Mode will now be available for selection. When a Mission is 
    started this toggle will be disabled (ie. the Operator will not be able 
    to modify/add Viapoints).
...
13. **Waypoint Panel Toggle:** View list of Goals in the current mission.

14. **Waypoint Drop Button:** The Waypoint indicator marker on the
    bottom bar allow the user to place a Waypoint at the location of the UGV. 

15. **Start Button:** Start the current mission (ie. execute the list of
    enabled Goals sequentially).

16. **Pause Button:** Pause the current mission. Pressing the start button
    while paused will continue the current mission.

17. **Stop Button:** Cancel the Mission or Task that is currently being
...
6.  **Datum:** The blue Waypoint marker on the map view represents the
    ...
    ENU convention.
7.  **Scale:** The scale representing the ratio of a distance on the map
