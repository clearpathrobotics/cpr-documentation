...
3.  Select the Task from the dropdown list. Standard waypoint icons will be
    ...
    -   <img src="/img/outdoornav_images/customTask_40px.png" width="30px"/> <strong>New Custom Task: </strong>
        ...
        for more details on setting up custom tasks.
...
In the case where users wish to perform custom tasks on their UGV setup the custom_task_manager is available. Users can create their own 
tasks following a specific template and place them within the **/onav_log/custom_tasks** directory.  Restarting the UGV or dockers will trigger the system
to load the custom task that was created, making it available for mission use.  Below is a sample template file named "log_inputs.py" which 
...
from onav_custom_tasks.custom_task_base import *

class CustomTask(CustomTaskBase):
  def __init__(self):
      super().__init__("log_inputs")

  def run_task(self, goal):
    if len(goal.strings) == 0 or len(goal.floats) == 0:
...
variables have any data in them the task is aborted.
...
To leverage custom tasks users will need to create a new file that defines a new class called `CustomTask` which inherits from 
`CustomTaskBase`. The `CustomTaskBase` class is found in the `onav_custom_tasks.custom_task_base` package. It exposes a `SimpleActionServer` 
object as `_as`, a `UITaskFeedback` object as `_feedback` and a `UITaskResults` object as `_result` to be used as part of the tasks functionality. The custom 
task manager requires the user to also define a `run_task` method that takes a `UITaskGoal` object as a parameter.
