...
found in the `onav_tasks.custom_task_base` package. It exposes a `SimpleActionServer` (see <a href="http://wiki.ros.org/actionlib_tutorials/Tutorials/Writing%20a%20Simple%20Action%20Server%20using%20the%20Execute%20Callback%20%28Python%29">here</a> 
...
Users can set the feedback state using the `SimpleActionServer` class's standard methods. In this case the task iterates through the comma delimited
...
By selecting the "SCHEDULER" option from the upper left hand hamburger menu, the user can open the Mission scheduler interface. This interface 
allows users to manage schedules that will automatically run missions based off of specific times and number of iterations. The view can be found 
in this screen shot and is elaborated further afterwards.
...
The user can either create a new schedule by filling in the appropriate fields or they may edit a schedule by selecting the pencil icon for that schedule 
in the bottom table.  The input fields are outlined as follows:

-   <strong>Name</strong>: The name of the schedule.
-   <strong>Start Time</strong>: The local time that the schedule will start. This is stored as UTC time on the robot.
...
-   <strong>Loop Interval</strong>: The amount of time that should take place inbetween loops. This should include the execution time of the schedule. 
    For example if the schedule takes 5 minutes to run and this value is set to 10 minutes, then the schedule will start the next loop 5 minutes after completing the previous loop.
