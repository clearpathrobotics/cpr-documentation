...
-   **Ghost Waypoint** A transparent waypoint that is not part of the mission. This
    Waypoint appears inbetween two other waypoints when in edit mode. The user can 
    drag and drop this ghost waypoint to add a new waypoint to the mission in between
...
To create Missions while edit mode is enabled select the "Waypoint Mode"
button. This will allow the user to place Waypoints at locations where
the user clicks on the map. These will appear as red Waypoints with the 
exception of the first waypoint (green) and the last waypoint (yellow).

As waypoints are placed, a "ghost waypoint" will appear between each real
waypoint and can be dragged to a new spot to insert a real Waypoint in 
...
enable the "Waypoints Panel" toggle to access the list of Goals. Here, the
...
1.  Enable the "Waypoint Panel" toggle. If you cannot find this button,
    see [Waypoint Panel](#waypoint-panel) for reference.
...
for the Waypoint. For example, when creating a mission to an inspection point,
...
waypoint). If you need the robot to be at specific headings at
other waypoints you'll need to enter these in for each specific Waypoint.
...
The user can save the Mission at any time while the UI is
open in the web browser. When the user clicks the "Save" button (see
[Main Components](/docs_outdoornav_user_manual/web_user_interface/ui_overview#main-components) the Mission will be
...
-   Enable the "Edit Mission" toggle. Select the "Waypoint Mode"
    button.
-   Select the Waypoint icon on the bottom bar 
    to create a Waypoint at the current location of the UGV. This step should
    either be it's own mission or it should be the starting point of a mission.
