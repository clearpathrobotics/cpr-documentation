...
To create a new Mission to run on the UGV first ensure that the UI is in "Edit Mode" (
select the pencil icon in the bottom bar). Then open the drop down menu in the bottom
bar and select the "Add Mission" option. This will allow you to create a new mission 
which you can then add Waypoints to. 
...
require that the position and/or orientation at a Waypoint be very accurate,
