...
navigation modules that allows users to predefine drivable paths and 
...
there relevant components. These terms are referred to throughout this manual.

-   **Mission** A Mission is a set of one or more Waypoints.
-   **Map** A user created map that goes over the satellite view and is used to define valid drivable 
    areas for a mission.
...
Before creating an missions the user will need to first create a map that the UGV can use 
to generate paths.  First ensure that the UI is in "Map Edit Mode" (select the pencil icon in the bottom bar next to
the map selection drop down). The open the drop down menu and select the "Add Map" Option.  From here the user can create 
...
map details. The user can modify the maps name, defaults and edit path's through this 
view.  
...
clicking the "Play" button <img src="/img/outdoornav_images/ui_start_button.png" width="16px"/>. 
