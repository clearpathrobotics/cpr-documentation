---
title: Husky A200 Observer Tutorials
sidebar_label: Tutorials
sidebar_position: 3
---

import ComponentIntroductionHuskyObserver from "/components/introduction_husky_a200_observer.mdx";
import Support from "/components/support.mdx";

<ComponentIntroductionHuskyObserver />

## Husky A200 Observer Overview

### Introduction

Husky A200 Observer is a fully integrated system that enables robotics developers to accelerate inspection solutions.
It is built on top of the versatile [Husky](../../../ros1_robots/outdoor_robots/husky/user_manual_husky.mdx) base platform
The tutorial topics are listed in the right column and presented in the suggested reading order.

For more information or to receive a quote, please [visit us online](http://clearpathrobotics.com/husky).

:::note

These tutorials assume that you are comfortable working with ROS. We recommend starting with our
[ROS tutorial](https://www.clearpathrobotics.com/assets/guides/noetic/ros/index.html) if you are not familiar with ROS already.

:::

:::note

The Husky A200 Observer is built on the the Husky platform. See also the [Husky Tutorials](../../../ros1_robots/outdoor_robots/husky/tutorials_husky.mdx#husky-ros-packages).

:::

---

## Using Husky A200 Observer {#using-husky-observer}

### Simulating Husky A200 
To simulate the Husky A200 Observer, including its full navigation stack, contact [Support](#support).

---

## Support {#support}

<Support />
