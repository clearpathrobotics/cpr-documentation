...
2. Install the SDK Manager by running the following commands:
   ...
   To resolve these, run `sudo apt install --fix-broken`.
   :::
...
1. Once the OS has been written to the Jetson, log into it and run the following commands to configure
   ...
   If `curl` is not installed on your Jetson by default you can install it by running `sudo apt install curl`.
...
See instructions [here](/docs/ros/installation/controller) for how to pair the PS4 controller.
