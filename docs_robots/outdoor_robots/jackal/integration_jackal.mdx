...
import ComponentStepModel from "/components/jackal_j100/jackal_j100_step_model.mdx";
...
While Jackal does not support [PACS&trade;](../../accessories/pacs) directly, the following kits may simplify your integration.

- [026369](/assets/pdf/clearpath_robotics_026369.pdf) Plate, which includes an 80 X 80 pattern of M5×0.8 threaded holes for mounting PACS&trade; sensors.
- [034769](/assets/pdf/clearpath_robotics_034769.pdf) Cable, helps add PACS&trade; sensors on a Jackal.
  Our PACS&trade; sensor kits include power cables that match Husky's connector and pinout.
  This adaptor cable converts Jackal's User Power connectors to have the same connector and pinout as Husky A300's.
  :::note

  The adaptor only includes 12 V and VBAT, since Jackal does not include a 24 V power supply.
  PACS&trade; sensor kits that use a 24 V pin will not work on Jackal by default.
