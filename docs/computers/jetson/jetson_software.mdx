...
a Jetson Xavier AGX, Nano, or TX2.
...
1.  Connect the Jetson to a PC running Ubuntu 20.04 using the provided microUSB cable.
...
3.  Connect a monitor, mouse, and keyboard to the Jetson. (The mouse is optional, but recommended.
    ...
    Jetson Jetson only has a single USB port.)
...
1. Download the latest version of [Nvidia's SDK Manager](https://developer.nvidia.com/nvidia-sdk-manager)
   onto the PC connected to the Jetson. Note that this PC must be running Ubuntu 20.04.
...
1. Back in your host PC, it will be waiting to install the extra SDK components on your Jetson.
...
2. When the Jetson starts up again, it should be connected to the robot. To see that the robot is connected by opening a
   ...
   Your Jetson should now be configured to operate as the robot's main PC.
...
To pair a PS4 controller to drive the Jetson, hold down the PS and Share buttons on the controller until
the light bar starts to flash. In a terminal on the Jetson, run `bluetoothctl` and then run the
following commands:
...
The light on the controller will turn solid blue once it is paired. With the controller paired you should
be able to control the robot by pressing L1 and using the left stick to drive. For more information see the
User Manual.

### Step 7: Use Remote PC with Jetson (Optional)

:::note
This step is optional.
:::

To use your Remote PC (eg. laptop) with the Jetson, note the IP address of the Jetson, install the robot-specific
desktop packages on the Remote PC, at set up your Remote PC to use the Jetson as the ROS master.
Refer to the "Installing Remote PC Software" section of your robot User Manual for details.
