...
| `/mcu/status`        | `dingo_msgs/Status`          | Low-frequency status data for Jackal's systems. This information is republished in human readable form on the `diagnostics` topic and is best consumed with the Robot Monitor.   |
...
1.  Using the provided PS4 controller.
    Refer to the [User Manual](/docs/robots/jackal/user_manual_jackal#controller) details on how to use the controller.
...
If you're working with a real Jackal, it's suggested to connect via SSH and launch the [jackal_navigation](https://github.com/jackal/jackal/tree/noetic-devel/jackal_navigation) launch files from on board the robot.
...
- `spawn_dingo.launch`, which is intended to be included in any custom world to add a Jackal simulation to it.
...
For more information on ROS on Windows, please visit [Microsoft's ROS documentation](http://aka.ms/ros).
...
1. You can [download a trial](https://www.microsoft.com/en-us/evalcenter/evaluate-windows-10-enterprise) of Windows 10 IoT Enterprise Long Term Service (LTSC) x64 build, use a Windows license from your Enterprise agreement, or a commercially purchased version of Windows 10 Desktop.
2. Create a [USB Boot stick](https://docs.microsoft.com/en-us/windows-hardware/manufacture/desktop/install-windows-from-a-usb-flash-drive) and copy the contents of the ISO generated from the image above.
3. Boot Windows and install following the prompts.
4. Install ROS on Windows using the [ROS Wiki instructions](https://wiki.ros.org/Installation/Windows).
...
If you have Windows specific questions or problems, Microsoft offers [community support through Github](https://github.com/ms-iot/rosonwindows) and [paid support](http://aka.ms/ros/support).
...
Windows provides an enterprise class network stack. To connect to a [Wi-Fi](https://support.microsoft.com/en-us/help/17137/windows-setting-up-wireless-network), use the Windows networking user interface to connect
to an access point.

#### Cellular Network Setup

Windows also provides cellular connectivity using an external Cellular modem. Microsoft provides documentation for
[consumer](https://support.microsoft.com/en-us/help/10739/windows-10-cellular-settings) and
[enterprise](https://docs.microsoft.com/en-us/windows/configuration/provisioning-apn) cellular deployments.
...
While many ROS packages are supported on Windows, most are not. Microsoft has provided [documentation](http://aka.ms/ros/docs) for
...
your own URDF which wraps the one provided by [jackal_description](http://wiki.ros.org/jackal_description).
