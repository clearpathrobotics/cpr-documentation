...
access when out of range of WiFi networks. A SIM card if not required if
Husky Observer is always being operated in the range of WiFi networks.
...
<center>
  ...
  <figure>
    <img
      ...
      width="400"
    />
    <figcaption>Cellular Only Mode</figcaption>
  </figure>
</center>
...
user may wish to adjust. The updates are optional
...
1. Configure a wired Ethernet port on your laptop to 192.168.131.100/24.
...
1. Open the following in your Chrome browser: http://192.168.131.50/network/wan<br/>
   This should bring you to the "Network -> WAN" menu.
...
1. Open the following in your Chrome browser: http://192.168.131.50/wireless/radio1<br/>
   This should bring you to the "Wireless -> Radio1" menu.
   Note: Refer [Appendix A](#networking-configuration) for default credentials.
...
1. See [Installation of SIM Card](#installation-of-sim-card) for details on the physical installation of a SIM card.
1. Configure a wired Ethernet port on your laptop to 192.168.131.100/24.
   See [Wired Network Connection](#wired-network-connection) for details.
1. Connect an ethernet cable from the configured port on your laptop to the
   debug/expansion port on the top of the robot.
1. Open the following in your Chrome browser: http://192.168.131.51/wwan/settings<br/>
   This should bring you to the "WWAN -> Settings" menu.
   Note: Refer [Appendix A](#networking-configuration) for default credentials.
...
This step is optional. It is only for customers attempting to access Husky Observer over the internet.
...
[ngrok](https://ngrok.com/) an application that enables developers to expose a local development server to
...
of customer configuration is required to use them.

1. [Sign up for ngrok](https://dashboard.ngrok.com/signup) if you don't yet have an account. The "Pro" and "Enterprise" tiers are recommended.
1. Configure a wired Ethernet port on your laptop to 192.168.131.100/24.
   See [Wired Network Connection](#wired-network-connection) for details.
1. Connect an ethernet cable from the configured port on your laptop to the
   debug/expansion port on the top of the robot.
...
once it has been surveyed (see above).
If a Base Station is not present, it is necessary to acquire RTK corrections through
...
1. [Create a Skylark subscription](https://sjphilli.account.swiftnav.com/subscriptions) from the subscriptions tab.
1. Create a new Device in the "Devices" tab corresponding to the robot that will be receiving the corrections.
1. TODO: details on updating the Duro units to point somewhere else? Or Skylark service to point to the robot?
