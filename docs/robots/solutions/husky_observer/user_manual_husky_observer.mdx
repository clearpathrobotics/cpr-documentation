...
1. "Base Station mode": Base station has wifi; robot has wifi + LTE (requires customer sim card); tablet has wifi connected to base station
1. "Customer Network mode": Customer wifi mesh network; robot has wifi + LTE (requires customer sim card); tablet has wifi connected to customer wifi network
1. "Cellular Only mode": No base station or customer wifi network, cellular in robot + separate cellular in tablet (built-in or via usb-tethered connection to cell phone)
...
TODO: add notes on Microhard setup in each case
