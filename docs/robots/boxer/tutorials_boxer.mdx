...
It is a lightly modified [OTTO 100](https://ottomotors.com/amrs) robot, equipped with an external PC mounted
...
[Driving Boxer](#driving-boxer) covers how to teleoperate Boxer using the remote control, a well as safety procedures for operating the physical robot.
...
The [boxer_description](https://github.com/boxer/boxer/tree/melodic-devel/boxer_description) repository provides a [URDF](http://wiki.ros.org/urdf) model of Boxer.
...
| `BOXER_API_VERSION` | `v1_1` | The version of the OTTO SDK running on the base platform. |
...
| `BOXER_PC` | `1` | If `1` the URDF will include a model of the backpack PC.  If `t` the backpack PC is omitted from the URDF.  The shape of the mode is determined by the `BOXER_PC_MODEL` variable |
| `BOXER_PC_MODEL` | `evs-2000` | Customizes the shape of the backpack PC added to the URDF. Other options are `ecx-2000` (for Vecow ECX-2000 series computer) and `mini-itx` (for common mini-ITX mini desktop enclosure) |
...
Your Boxer can be controlled through ROS, either through a ROS2 API on the base platform or through a ROS1 API on the optional backpack computer.
...
## Boxer Backpack PC Software Setup {#boxer-software-setup}

:::caution

All of the software instructions in this section apply only to the Backpack PC, not the
software running on the Base Platform. If your Boxer is not equipped with a Backpack PC,
...
The physical Boxer robot comes pre-configured with ROS and the necessary Boxer packages already installed on the backpack PC;
therefore, you will only need to follow the instructions below if you are re-installing software on the backpack PC.
...
During the installation process, you will then be asked to enter the robot's serial number. This is stamped on the
back of the robot, and will be of the form `A31_` followed by a series of numbers. It is important that this serial
number be entered correctly. Otherwise you may have errors communicating with the ROS2 API published by the base platform.
...
make sure to use the `eno1` Ethernet port on the backpack PC, not the Ethernet port on the base platform.
This `eno1` port is the same port normally used for communicating with the base platform. If eno1 (sometimes labelled LAN1)
is connected to the base platform, disconnect the base platform for now. Once the OS has installed you can reconnect the base
platform to `eno1`. You will also need to connect a monitor and keyboard to the backpack PC. We recommend connecting the Boxer
...
also equipped with an internal PC. This section explains how to configure the networking for the Backpack PC only. For
instructions on connecting the internal PC to a network, refer to the [OTTO 100 Documentation](https://help.ottomotors.com).

#### Enabling the OTTO App

Many of Boxer's features are accessible via the OTTO App. This requires configuring the base platform to connect
...
By default, Boxer backpack PC's Wi-Fi is in client mode, looking for the wireless network at the Clearpath factory.
...
1.  Using the provided PS4 controller.
    Refer to the [User Manual](/docs/robots/boxer/user_manual_boxer/#controller) details on how to use the controller.
...
Boxer uses [twist_mux](http://wiki.ros.org/twist_mux) to mix separate [geometry_msgs\Twist](http://docs.ros.org/api/geometry_msgs/html/msg/Twist.html) control channels into the `boxer_velocity_controller/cmd_vel` topic.

Additional velocity channels can be defined in [twist_mux.yaml](https://github.com/boxer/boxer/blob/melodic-devel/boxer_control/config/twist_mux.yaml), or can be piped into the lowest-priority `cmd_vel` topic.
...
For details on updating Boxer software or firmware, refer to [Software Maintenance](/docs/robots/boxer/maintenance_boxer#software_maintenance).

---

## Mapping and Navigation with Boxer {#navigating-boxer}
...
If you're working with a real Boxer, it's suggested to connect wirelesslyvia SSH and launch the ROS launch files from onboard the robot.
