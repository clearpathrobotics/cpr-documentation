...
For details on the ROS2 API, please contact our Support department.
...
Please take time to locate the red Motion Stop button on the rear-right of the system.
When the Motion Stop button pressed, it provides a secure way to stop the robot movement, but does not control power attached by the attachment panel.
Ensure the Motion Stop button is accessible at all times.
