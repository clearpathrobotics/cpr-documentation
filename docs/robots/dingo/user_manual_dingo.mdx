...
-   **battery indicator**: LED is solid green when the battery level is ok (TODO: above 30% ?), solid red when the battery
    level is getting low (TODO: between 10-30%(?) -- start charging soon), and flashing red when the battery level is
    critical (TODO: below 10%(?) -- begin charging immediately);
...
on adding an external stop button/switch. Contact [Support](#dingo-support) for more information.
...
Similar to the data connections, the power lead may be brought through the trough cover and connected
to the user power: unregulated battery power, regulated 12V power, or regulated 5V power.
...
Use DigiKey part WM3701-ND for connecting to 12V/5V power and DigiKey part WM10378-ND for connecting
to VBAT power. Route the power from the MCU board to the appropriate location, either to the internal
...
Electrically, the `INPUTS` connector (TODO: part number; mating connector TODO)
is used for IN1 and IN2, with pin mapping as
...
Electrically, the `SW_IN` connector (TODO: part number; mating connector TODO)
is used for IN3, with pin mapping as outlined
...
Each of the AUX outputs is a 4-pin connector (TODO: part number; mating connector TODO)
with pin mapping as outlined below. 
...
connector on the MCU labelled `RELAY` (TODO: part number; mating connector TODO)
with pin mapping:
...
Electrically, connect a 12V 2-pin fan to the MCU connector (TODO: mfg part number) labelled `FAN`
using a TODO mating connector. The pins should be connected as follows:
...
functionality. Electrically, the external e-stop button can be connected to the 6-pin MCU connector (TODO: mfg part number)
labelled `ESTOP` using a TODO mating connector.
