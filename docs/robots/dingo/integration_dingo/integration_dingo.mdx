...
A standard Dingo is delivered with an 80 mm square mounting hole pattern on Dingo's
trough cover as shown in [Exterior Features](#dingo-exterior-features), which can be
used for attaching external payloads. The mounting holes come with M5 screws pre-installed.
You may mount your hardware directly onto the trough cover or you may design and mount
a new plate to the trough cover and secure it to the trough cover using M5 screws.
...
You can also try different confgurations using our tool at [build.clearpathrobotics.com/husky](https://build.clearpathrobotics.com/dingo)
...
The following are available as user power: unregulated battery power (see VBAT), regulated 12 V power
...
outlined below. Note that each of the two inputs can be 12 V or 24 V inputs, with a
...
Electrically, connect a 12 V 2-pin fan to the MCU connector labelled `FAN`
...
In addition to the built-in Motion Stop button, an external motion-stop button can be added that has similar
functionality. Electrically, the external motion-stop button can be connected to the 6-pin MCU connector
labelled `ESTOP` using a mating connector.

The six pins should be connected as follows:

- 1-2: to external motion-stop breakout: press and release external button (3.3V) to engage motion-stop; press and release
  external button or Motion Stop button to clear the motion-stop
