...
up to the maximum output current of the charger. If total power draw is above the output current of the charger,
...
1. Pres the "PS" button on the joystick controller to connect it to the robot.
...
1.  When battery is charged fully, the LED on the Standard Charger will illuminate green if the the robot is powered off.
    Alternative, if the robot is powered on the LED on the Standard Charger will remain red and Husky's [Status Lights](#status-lights)
...
**Never push Husky A300 using a powered device at speed above 2 m/s, such as
