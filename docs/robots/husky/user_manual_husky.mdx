...
Dingo is built around a computer running Ubuntu (Intel-based computer or Jetson developer kit), paired
with a 32-bit MCU. The MCU handles I/O, power supply monitoring, and motor control, as well as supplying
data from the integrated IMU. The communication channel between the MCU and computer is a serial connection.
...
To begin, place Husky "up on blocks" as described in the [Safety](#safety) section,
making sure that the wheels are clear of the ground.

For most Husky setups, there will be an Onboard Computer which is directly connected to the Husky
and a Remote Computer which is used to control Husky and gather data. These two computers must be set up
to communicate with each other.

### Onboard Computer Setup

If you purchased an Onboard Computer from Clearpath Robotics with Husky, it is already installed,
connected, and powered. If not, or to set up your own Onboard Computer, see the
[Husky Tutorials](/docs/robots/husky/tutorials_husky).

Note that the Onboard Computer must be connected to Husky's serial port, which is located in the
...
The connector is a female DE-9, suitable for connection directly to a USB-serial adaptor or to a computer
serial port via a straight-through modem cable (not a null modem cable). How you connect the Onboard
Computer to Husky depends on whether the Onboard Computer has a dedicated serial port. Husky is delivered with a
USB-serial adaptor.
...
To get Husky connected to your local Wi-Fi network, you must first access the Onboard Computer using
a wired connection to the robot. See the
[Husky Tutorials](/docs/robots/husky/tutorials_husky#husky-networking) for details.

### Connecting and Using the Controller {#controller}

To use your controller, it must be paired with your Husky and powered on. If your purchased the Onboard Computer from
