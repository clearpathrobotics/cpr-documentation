...
A red OMM light means the Husky's computer to microcontroller serial connection is not extablished.
...
- Funcioning connections from the USB-Serial cable to the MCU's microcontroller. (soldered connectors, PCBA traces, microcontroller's soldered pins)
