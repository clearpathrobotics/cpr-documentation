...
These motor controllers change the voltage across the DC brushed motors, causing a mechanical torque at the wheel.
...
This is why your Husky will roll on a slope, since it is just the resistive power of the windings that applying a small torque.
...
- Husky's motor controllers are receiving a zero velocity command, rather than a small value like 0.005 m/s.
