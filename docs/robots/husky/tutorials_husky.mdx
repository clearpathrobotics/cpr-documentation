...
1. Using the provided PS4 or Logitech controller. Refer to the [User Manual](/docs/robots/husky/user_manual_husky/#controller)
...
[localization.yaml](https://github.com/husky/husky/blob/noetic-devel/husky_control/config/twist_mux.yaml).
...
1. Using the interactive remote controller in RViz. See [Simulation](#simulating-husky).
2. Using autonomous navigation. See [Navigation](#navigating-husky).
...
To drive the Husky, Axis 0 controls the robot's steering, Axis 1 controls the forward/backward velocity,
and buttons 4 and 5 act as enable and enable-turbo respectively. On common controllers these correspond to
the following physical controls:

| Axis/Button | Physical Input                   | PS4 | F710 | Xbox One | Action                 |
| :---------- | :------------------------------- | :-- | :--- | :------- | :--------------------- |
| Axis 0      | Left thumb stick horizontal      | LJ  | LJ   | LJ       | Drive forward/backward |
| Axis 1      | Left thumb stick vertical        | LJ  | LJ   | LJ       | Rotate/turn            |
...
scanner publishing on the `/scan` topic.
...
on a simulated Husky, or a factory-standard Husky with a laser scanner publishing on the `/scan` topic.
...
- **E-Stop Test**
    ...
    Checks that the robot's E-Stop is working properly. 

    This test subscribes to the `/status` ROS topic and checks that when the E-Stop is manually engaged by
    the user, the E-Stop state is correctly reported on the `/status` ROS topic.
...
- **Drive Test**
    ...
    Drives the robot forward 1 meter and checks that the motors, encoders, and encoder-fused odometry are working properly.

    This test:

    - Subscribes to the `/husky_velocity_controller/odom` ROS topic to receive linear displacement estimations from the
      encoder-fused odometry. The robot will drive forward until 1 meter is estimated.
...
Refer to the [Jetson](/docs/computers/jetson) page for details on installing a Jetson in Husky.
