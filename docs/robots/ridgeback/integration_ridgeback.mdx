...
Ridgeback's internal computer options support USB3 and Ethernet connectivity. Where available, the
...
- For the High Current VBAT User Power (labelled as 24 VDC, 20A MAX), terminate your payload's power lead with the appropriate Anderson connectors.
  TODO: why is it a 7.5A fuse in figure??
...
- For the Inverter Power, terminate your payload's power lead with the appropriate ??? connectors.
...
- Unregulated VBAT power (25.6 V nominal, 16 A max), shared across the High Current VBAT User Power (20 A max)
  and User Power Ports (5 A max per port)
- Regulated 12 V power (?? A max), shared across the User Power ports (5 A max per port)
- Regulated 5 V power (?? A max), shared across the User Power ports (5 A max per port).
...
Electrically, the external motion-stop button can be connected to the 4-pin MCU connector labelled `E-STOP BREAKOUT` using
