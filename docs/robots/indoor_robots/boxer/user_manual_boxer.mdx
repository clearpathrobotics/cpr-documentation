...
with using your robot. Note that the front e-stop button is only present on Boxer V2.5.
...
In addition to the Rear E-Stop button described in the HMI Panel above, Boxer 2.5 (but not Boxer 2.4)
has a Front E-Stop button, which is mounted to the right of the front safety LiDAR. Refer to
[E-Stop Buttons](#boxer-stop-buttons) for additional details.
...
2.  Ensure that all E-Stop buttons are released by pulling out the red buttons at the front (V2.5 only) and rear of the unit, and then pressing the blue E-Stop Reset button.
    <center>
      <img
        src="/img/robot_images/boxer_images/robot_boxer_13.jpg"
        width="200"
...
3.  Release all E-Stop buttons and press the E-Stop Reset button.
...
### E-Stop Buttons {#boxer-stop-buttons}

:::warning

Ensure the E-Stop buttons are accessible at all times.
Avoid mounting payloads that extend over the front or rear of Boxer and would occlude the E-Stop buttons.

:::

Please take time to locate the red E-Stop buttons on the front (V2.5 only) and rear and of the system.
...
When any E-Stop button is pressed, it provides a secure way to stop the robot movement, but does not control power attached by the attachment panel.
The e-stop function can be reset by pulling out all the red E-Stop buttons until they return to an extended position and then pressing the E-Stop Reset button.
