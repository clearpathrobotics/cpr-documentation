...
[Driving Warthog](#driving-warthog) covers how to teleoperate Warthog using the remote control, a well as safety procedures for operating the physical robot.
...
If this is your first unboxing, ensure that Warthog's wireless antennae are firmly screwed on to the chassis.
...
1.  Using the provided Fubata controller.
    Refer to the [User Manual](/docs/robots/warthog/user_manual_warthog/#controller) details on how to use the controller.
...
Diagnostics are only applicable to real Warthog robots, not simulation.
...
Refer to [Pinch Points](/docs/robots/warthog/user_manual_warthog#warthog-pinch-points) for
...
For details on updating Warthog software or firmware, refer to [Software Maintenance](/docs/robots/warthog/maintenance_warthog#software_maintenance).
...
Based on the Warthog configuration, either Warthog-D or Warthog-O, this script will know to check for good CAN packets from 2 or 4 encoders, respectively.
...
link, offset 5 cm towards the front of the robot.
