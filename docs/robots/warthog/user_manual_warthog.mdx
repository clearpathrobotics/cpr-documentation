...
The Warthog exterior is shown below and includes:
...
- Four status lights (one on each corner)
...
See http://wiki.ros.org/warthog_base for information on commanding the status lights.

| Light Output             | System State                                                                 |
| :----------------------- | :-------------------------------------------------------------------------------- |
...
| Flashing red             | The Stop circuit is broken. Twist the mushroom buttons to ensure that they are unlatched, and check any external stop hardware, if present. |
| Flashing yellow, corners | All stops are closed but not reset; the stop loop is now only electrically latched. Press the Stop Reset button to enable full operation.  |
| Flashing yellow, all     | Motor drivers not yet ready to drive. The motors have a brief initialization sequence that must complete after a stop condition clears before they are ready to drive. If this condition persists, please contact support.  |
...
| Headlights + taillights  | When Ridgeback is ready to drive, the front lights will change from red to white. The intensity of the head and tail lights will increase slightly when actually in motion. This is the status which may be overridden by publishing your own light patterns to the `/cmd_lights` ROS topic.  |
