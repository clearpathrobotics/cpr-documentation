...
`netplan` configurations. The `clearpath_comptuer_setup` package is available on the [Clearpath package server](../installation/robot.mdx#clearpath-package-server).
